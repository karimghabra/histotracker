          ...
          {samples.map((sample) => (
            <div key={sample.id} className="flex items-center gap-2 rounded-md border border-line bg-surface px-2.5 py-2">
              ...
              {/* No `samples.length > 1` gate any more (#135). Hiding the
                  ...
                  in the record. Taking the last one out ends the run: a planned
                  run is removed, a started one is cancelled and kept (#148). */}
              {canEditMembers && (
                <button
                  title={
                    samples.length === 1
                      ? isRunning
                        ? `Take ${displayCode(sample.sample_code)} out - this cancels the run and keeps its record`
