...
// OPEN ISSUE #141: remove `.fails` in the pull request that fixes it.
it.fails("#141: backspacing and retyping a hex digit edits the field and then the palette", async () => {
...
// OPEN ISSUE #141: remove `.fails` in the pull request that fixes it.
it.fails("#141: a field cleared to start again can be typed into from '#'", async () => {
  const user = userEvent.setup();
  render(<Harness seen={[]} />);
  const field = screen.getByLabelText("Surface hex") as HTMLInputElement;
