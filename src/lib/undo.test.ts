...
const snap = (label: string, n: number): Snapshot => ({ label, bytes: [n] });
...
describe("undo store — whole-DB snapshot bookkeeping", () => {
  ...
  it("undo returns the recorded pre-state and moves the current state to redo", () => {
    ...
    expect(entry?.bytes).toEqual([1]);
    expect(useUndoStore.getState().undoStack).toHaveLength(0);
    expect(useUndoStore.getState().redoStack[0].bytes).toEqual([2]);
  });
  ...
  it("redo returns the post-state and stashes the current back onto undo", () => {
    ...
    expect(entry?.bytes).toEqual([2]); // redo restores the post-state
    expect(useUndoStore.getState().redoStack).toHaveLength(0);
    expect(useUndoStore.getState().undoStack[0].bytes).toEqual([1]);
  });
